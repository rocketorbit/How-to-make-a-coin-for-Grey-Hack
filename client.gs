...
features["balance"]["run"] = function()
    ...
    if not subWallet.check_password(subWalletPass) then user_input("Error: invalid user or password.", false, true)
...
features["transaction"]["run"] = function()
    ...
    if not subWallet.check_password(subWalletPass) then user_input("Error: invalid sender/reciever/password.", false, true)
